...
<ParamField path="trace_id" type="string" required>
  ...
  **Format:** Any string, typically `tr-{random_number}` or UUID format.

  **Example:** `"tr-1764388648079963068"`

  <Note>
    The trace_id is echoed back in the response for request correlation.
  </Note>
</ParamField>
...
<ParamField path="code" type="string" required>
  ...
  **Notes:**
  - Must be valid syntax for the specified language
  - Newlines should be `\n` characters
  - Special characters should be properly escaped in JSON
  - No size limit, but practical limit based on memory
...
<ParamField path="timeout" type="integer" default="10">
  ...
  **Behavior:**
  - Execution is killed after timeout
  - Response includes `exit_code: 124`
  - stderr includes "Execution timed out"
