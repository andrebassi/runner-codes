...
<Card title="SWI-Prolog" icon="brain">
  SWI-Prolog interpreter for logic programming.
</Card>
...
```bash
# 1. Create Rootfs from Docker (custom build with swi-prolog)
# Build: FROM debian:bookworm-slim + apt-get install swi-prolog
