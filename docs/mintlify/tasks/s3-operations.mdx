...
```bash
# Creates Python rootfs AND uploads to S3
task aws:create-rootfs-python

# Creates all rootfs images AND uploads each to S3
task aws:create-all-rootfs
```

<Accordion title="Creation + Upload Flow">
```
...
# Create symlink to default rootfs
...
<Frame>
  <img src="/images/s3-workflow.svg" alt="S3 Storage Workflow" />
</Frame>
