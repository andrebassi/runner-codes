...
description: 'MariaDB SQL syntax validation without database connection'
---

## Overview

MariaDB Parser validates MariaDB SQL syntax using [node-sql-parser](https://github.com/taozhi8833998/node-sql-parser) WITHOUT requiring a real MariaDB database.

## Specifications

| Property | Value |
|----------|-------|
| Language ID | `mariadb-parser` |
...
```bash title="Create working directory for MariaDB parser"
mkdir -p /tmp/docker/mariadb-parser
cd /tmp/docker/mariadb-parser
```

### Step 2: Create mariadb-run.js Script

```bash title="Create Node.js parser wrapper script"
cat > mariadb-run.js << 'EOF'
#!/usr/bin/env node
const fs = require('fs');
...
function main() {
    ...
    if (args.length === 0) {
        console.error('Usage: mariadb-run.js <file.sql>');
        process.exit(1);
    }
...
COPY mariadb-run.js /usr/local/bin/mariadb-run
RUN chmod +x /usr/local/bin/mariadb-run
...
docker build -t ttl.sh/llm-fc-mariadb-parser:24h .
docker push ttl.sh/llm-fc-mariadb-parser:24h
```
...
sudo infra.operator rootfs from-docker \
    --name mariadb-parser \
    --image ttl.sh/llm-fc-mariadb-parser:24h \
    --size 500
```
...
sudo -E infra.operator rootfs upload --lang mariadb-parser --bucket llm-firecracker
sudo -E infra.operator snapshot upload --lang mariadb-parser --bucket llm-firecracker
```
...
```json title="Request"
{
  "trace_id": "mariadb-hello-001",
  "lang": "mariadb-parser",
  "code": "SELECT 'Hello from MariaDB Parser!' AS message;",
  "timeout": 30
}
```
...
{
  "trace_id": "mariadb-hello-001",
  "stdout": "=== SQL Validation Result ===\nStatus: VALID\nDatabase: MariaDB\n\n=== Formatted SQL ===\nSELECT 'Hello from MariaDB Parser!' AS `message`\n\n=== Statement Summary ===\nTotal statements: 1\n  1. select\n",
  "stderr": "",
  "exit_code": 0
}
```
...
```json title="Request"
{
  "trace_id": "mariadb-upsert-001",
  "lang": "mariadb-parser",
  "code": "INSERT INTO users (id, name, email) VALUES (1, 'John', 'john@example.com')\nON DUPLICATE KEY UPDATE name = VALUES(name), email = VALUES(email);",
  "timeout": 30
}
```
...
{
  "trace_id": "mariadb-upsert-001",
  "stdout": "=== SQL Validation Result ===\nStatus: VALID\nDatabase: MariaDB\n\n=== Formatted SQL ===\nINSERT INTO `users` (`id`, `name`, `email`) VALUES (1, 'John', 'john@example.com') ON DUPLICATE KEY UPDATE `name` = VALUES(`name`), `email` = VALUES(`email`)\n\n=== Statement Summary ===\nTotal statements: 1\n  1. insert\n",
  "stderr": "",
  "exit_code": 0
}
```
