...
```bash title="Create working directory for SQLite parser"
...
sudo -E infra.operator rootfs upload --lang sqlite-parser --bucket runner-codes
sudo -E infra.operator snapshot upload --lang sqlite-parser --bucket runner-codes
```
