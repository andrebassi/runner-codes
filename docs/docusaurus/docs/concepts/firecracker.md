...
![Firecracker Architecture](/img/firecracker-architecture.svg)
...
```json title="Machine configuration"
{
  "vcpu_count": 1,
  "mem_size_mib": 512,
  "smt": false
...
```json title="Boot source configuration"
{
...
```json title="Drive configuration"
{
  "drive_id": "rootfs",
...
```go title="Start Firecracker VM from Go"
...
```bash title="Load modules if missing"
sudo modprobe kvm
